...
const repl = `                        {/* Feel Lanka — scattered animated line */}
                        <div style={{marginBottom:'1.4rem',lineHeight:1.05, textAlign:'center', width:'100%'}}>
                            <style>
                                {\`
                                @keyframes scatterIn {
                                    0% {
                            ...
                            <div id="feel-lanka-headline" style={{fontFamily:"'Outfit',sans-serif",fontSize:'clamp(1.4rem,3vw,2.4rem)',fontWeight:600,color:'rgba(255,255,255,0.82)',display:'inline-block',margin:'0 auto',letterSpacing:'0.04em',whiteSpace:'nowrap', textAlign:'center'}}>
                                {"Feel Lanka — Beyond Travel, Into Experience.".split(' ').map((word, wIndex, arr) => (
                                    <span key={wIndex} style={{ display: 'inline-block', whiteSpace: 'nowrap', marginRight: wIndex !== arr.length - 1 ? '0.3em' : '0' }}>
...
if (regex.test(normC)) {
    ...
    console.log("Success with Centering Fix");
} else {
